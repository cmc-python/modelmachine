...
class TestControlUnitM(TCu2):
    ...
    def run_fetch(self, value, opcode, instruction_size, *, r2=True):
        ...
        if r2:
            self.registers.fetch.assert_has_calls(
                [call("PC", 2 * BYTE_SIZE), call("R2", WORD_SIZE)]
            )
        else:
            self.registers.fetch.assert_any_call("PC", 2 * BYTE_SIZE)
        self.registers.put.assert_has_calls(
            [
                call("RI", value, WORD_SIZE),
    ...
    def test_load(self):
        ...
        for opcode in ARITHMETIC_OPCODES | {OP_LOAD, OP_COMP}:
            ...
            self.registers.put.assert_has_calls(
                [call("S", val1, WORD_SIZE), call("RZ", val3, WORD_SIZE)]
            )
        ...
        for opcode in REGISTER_OPCODES:
            ...
            self.control_unit.load()
            self.registers.fetch.assert_has_calls(
                [call(register1, WORD_SIZE), call(register2, WORD_SIZE)]
            )
            self.registers.put.assert_has_calls(
                [call("S", val1, WORD_SIZE), call("RZ", val2, WORD_SIZE)]
            )
    ...
    def run_write_back(self, should, opcode):
        ...
        if should == "two_registers":
            self.registers.fetch.assert_has_calls(
                [
                    call(res_register1, self.operand_size),
                    call(res_register2, self.operand_size),
                ]
            )
            self.registers.put.assert_has_calls(
                [
                    call(register1, val1, self.operand_size),
            ...
            self.registers.fetch.assert_called_once_with(
                res_register1, self.operand_size
            )
            self.registers.put.assert_called_once_with(
                register1, val1, self.operand_size
            )
            assert self.ram.fetch(address, self.operand_size) == canary
