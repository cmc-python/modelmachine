...
class ControlUnit3(ControlUnit):
    ...
    NAME = "mm-3"
    KNOWN_OPCODES = ARITHMETIC_OPCODES | JUMP_OPCODES | {Opcode.move, Opcode.halt}
    ...
    def _address1(self) -> Cell:
        return self._ir[2 * self._ram.address_bits : 3 * self._ram.address_bits]
    ...
    def _decode(self) -> None:
        ...
        if self._opcode is Opcode.move:
            self._expect_zero(self._ram.address_bits, 2 * self._ram.address_bits)

    _LOAD_R1R2: Final[frozenset[Opcode]] = ARITHMETIC_OPCODES | CONDJUMP_OPCODES
    ...
    def _write_back(self) -> None:
        ...
        if self._opcode in self._WB_OPCODES:
            self._ram.put(address=self._address3, value=self._registers[RegisterName.S])
