...
* Подумать об остановки halt
* Добавить word_size в io_unit
...
* `cu.py` *не реализованно* - контролирующее устройство, выполняющее
...
обращайтесь к документации этих конкретных модулей и их исходным кодам.
...
`get_value` и `get_data`. Округление производится в сторону нуля.
...
* `IP` *только* для пересылки туда адреса из регистра `R1` при прыжках

Схема работы:

* Арифметические комманды: `add`, `sub`, `smul`, `sdiv`, `umul`, `udiv`.
  Они работают следующим образом: `S := R1 op R2`.
...
1. `fetch_and_decode` - загрузка и расшифровка очередной команды.
   Обычно содержимое ячейки оперативной памяти с адресом записанным
...
* `run_file` - загрузка, запись и печать
